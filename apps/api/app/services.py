...
from .cache import bump_rate_limit, cache_dashboard, invalidate_dashboard_cache, invalidate_live_dashboard_cache, read_dashboard_cache
...
def _invalidate_all_dashboard_caches(include_live_dashboard=False):
 users = fetch_all("select id from users")
 for row in users:
  invalidate_dashboard_cache(row["id"])
  if include_live_dashboard:
   invalidate_live_dashboard_cache(row["id"])
 ...
 with get_connection() as conn:
  with conn.cursor() as cur:
   cur.execute(
    ...
    (_id("audit"), actor_user_id, action, entity_type, entity_id, json.dumps(payload or {})),
   )
...
def current_user_from_token(session_token):
 ...
 row = fetch_one(
  "select u.id, u.email, u.name, u.role, u.onboarding_completed, u.email_verified_at from sessions s join users u on u.id = s.user_id where s.token_hash = %s and s.revoked_at is null and s.expires_at > now()",
  (token_hash,),
 )
...
def latest_regime():
 ...
 return {
  ...
  "components": [{"key": item["key"], "label": item["label"], "value": float(item["value"])} for item in components],
 }
...
def list_briefings():
 ...
 return [
  {"id": item["id"], "slug": item["slug"], "title": item["title"], "kind": item["kind"], "publishedAt": item["published_at"].isoformat(), "summary": item["summary"], "analystName": item["analyst_name"], "takeaways": item["takeaways"], "assetSymbols": item["asset_symbols"], "relatedEventId": item["event_id"]}
  for item in rows
 ]
...
def list_alerts(user_id):
 ...
 return [
  {"id": item["id"], "name": item["name"], "triggerType": item["trigger_type"], "deliveryChannel": item["delivery_channel"], "status": item["status"], "threshold": item["threshold_value"], "lastTriggeredAt": item["last_triggered_at"].isoformat() if item["last_triggered_at"] else None}
  for item in rows
 ]
...
def list_posts():
 ...
 return [
  {"id": item["id"], "title": item["title"], "body": item["body"], "authorName": item["name"], "authorRole": item["role"], "likes": int(item["likes"]), "comments": int(item["comments"]), "createdAt": item["created_at"].isoformat()}
  for item in rows
 ]
...
def create_post(user_id, payload):
 ...
 _invalidate_all_dashboard_caches(include_live_dashboard=True)
 return post_id
...
def _build_workstation_payload(user):
 payload = {
  ...
  "billing": billing_state(user["id"]),
  "adminSummary": admin_summary() if user["role"] == "admin" else None,
 }
