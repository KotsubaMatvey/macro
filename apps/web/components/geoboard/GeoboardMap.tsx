...
export function GeoboardMap(props: { mode: GeoboardMode; gdeltEvents: any[]; macroEvents: any[]; zones: RegimeZone[]; pulseId: string | null; focusTarget: { lat: number; lon: number; zoom: number } | null; onHoverChange: (hover: HoverState | null) => void }) {
 ...
 useEffect(function () { fetch('/geo/countries-110m.json').then(function (response) { return response.json() }).then(setGeoData).catch(function () { setGeoData(null) }) }, [])
 ...
 return <div className='relative h-full w-full bg-[#060a0f]'><DeckGL viewState={viewState} controller layers={layers} onViewStateChange={function (params: any) { setViewState(params.viewState) }} onHover={function (info: any) { if (!info.object || !info.layer) { props.onHoverChange(null); return } const id = layerName(String(info.layer.id)); if (!id) { props.onHoverChange(null); return } props.onHoverChange({ layer: id as HoverState['layer'], x: info.x, y: info.y, object: info.object } as HoverState) }}><Map reuseMaps mapboxAccessToken={token} mapStyle='mapbox://styles/mapbox/dark-v11' attributionControl={false} /></DeckGL>{!token ? <div className='absolute inset-x-4 top-4 border border-[#f43f5e]/35 bg-[#060a0f] px-3 py-2 text-[10px] uppercase tracking-[0.1em] text-[#fda4af]'>NEXT_PUBLIC_MAPBOX_TOKEN missing //</div> : null}</div>
}
