...
export async function PageShell(props: { title: string; subtitle: string; active: string; children?: Child }) {
 ...
 return h('main', { className: surfaces.page }, h('div', { className: surfaces.shell }, [
  h('aside', { key: 'sidebar', className: surfaces.sidebar }, [
   h('div', { key: 'brand', className: 'rounded-[15px] border border-white/8 bg-white/[0.025] px-3 py-3' }, [
    h('div', { key: 'eyebrow', className: 'text-[9px] font-semibold uppercase tracking-[0.24em] text-amber-300' }, 'Macro workstation'),
    h('div', { key: 'name', className: 'mt-2 text-lg font-semibold tracking-tight text-white' }, APP_NAME),
    h('div', { key: 'tagline', className: 'mt-2 text-[12px] leading-5 text-slate-400' }, 'Dense event, regime, bias, and reaction tooling for the operating desk.'),
   ]),
   h('div', { key: 'session', className: 'mt-3 rounded-[14px] border border-white/8 bg-white/[0.02] px-3 py-3' }, [
    h('div', { key: 'label', className: 'text-[9px] font-semibold uppercase tracking-[0.22em] text-slate-500' }, 'Active session'),
    h('div', { key: 'name', className: 'mt-2 text-sm font-medium text-white' }, session.name),
    h('div', { key: 'email', className: 'mt-1 text-[12px] text-slate-400' }, session.email),
   ...
   h('div', { key: 'label', className: 'mt-4 px-2 text-[9px] font-semibold uppercase tracking-[0.24em] text-slate-500' }, 'Navigation'),
   h('nav', { key: 'nav', className: 'mt-2 grid gap-1 overflow-y-auto pr-1' }, navItems.map(function (item) {
    ...
    return h(Link, { key: item.slug, href: '/app/' + item.slug, className: cx(surfaces.navItem, active ? surfaces.navItemActive : surfaces.navItemIdle) }, [
     h('div', { key: 'title', className: 'text-[13px] font-medium' }, item.title),
     h('div', { key: 'desc', className: cx('mt-1 text-[11px] leading-5', active ? 'text-slate-200' : 'text-slate-500 group-hover:text-slate-400') }, item.description),
    ])
   })),
   h('div', { key: 'footer', className: 'mt-4 rounded-[14px] border border-white/8 bg-white/[0.02] px-3 py-3' }, [
    ...
    h('div', { key: 'rows', className: 'mt-3 grid gap-2 text-[11px] text-slate-400' }, [
     h('div', { key: 'routing', className: 'flex items-center justify-between gap-3' }, [h('span', { key: 'l' }, 'Routing'), h('span', { key: 'r', className: 'ws-mono text-slate-200' }, 'dynamic')]),
     h('div', { key: 'jobs', className: 'flex items-center justify-between gap-3' }, [h('span', { key: 'l' }, 'Jobs'), h('span', { key: 'r', className: 'ws-mono text-slate-200' }, 'redis')]),
     h('div', { key: 'mode', className: 'flex items-center justify-between gap-3' }, [h('span', { key: 'l' }, 'Mode'), h('span', { key: 'r', className: 'ws-mono text-slate-200' }, 'provider-ready')]),
    ]),
   ]),
  ]),
