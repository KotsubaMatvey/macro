...
describe('ReportsPage', function () {
 it('renders archive, deterministic mode, and watch items', async function () {
  api.getReports.mockResolvedValue(reports)
  const view = await ReportsPage()
  render(view)
  expect(screen.getByText('Reports')).toBeInTheDocument()
  expect(screen.getByText('Report archive')).toBeInTheDocument()
  expect(screen.getByText('Latest briefs')).toBeInTheDocument()
  expect(screen.getByText('Deterministic weekly summary grounded in structured data.')).toBeInTheDocument()
  expect(screen.getAllByText('US CPI / DXY / Liquidity').length).toBeGreaterThan(0)
