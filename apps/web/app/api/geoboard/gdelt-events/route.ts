...
export async function GET() {
 ...
 try {
  const response = await fetch(API_URL + '/api/geoboard/gdelt-events', { cache: 'no-store', headers: { cookie: cookieHeader }, signal: controller.signal })
  ...
  if (error instanceof Error && error.name === 'AbortError') {
   console.error('GDELT proxy timed out', timeoutValue)
   return NextResponse.json({ detail: 'GDELT upstream timeout' }, { status: 504 })
  }
