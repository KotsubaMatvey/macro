...
export default async function LiveReactionsPage(props: { searchParams?: Promise<{ [key: string]: unknown }> }) {
 ...
 return h(PageShell, { title: "Reactions", subtitle: "Historical event-family reaction study using the live calendar and provider-backed market history where available.", active: "live-reactions", mode: payload.summary.freshness.mode === "live" ? payload.calendar.mode === "live" ? "live" : "mixed" : payload.summary.freshness.mode === "demo" ? payload.calendar.mode === "demo" ? "demo" : "mixed" : payload.calendar.mode === "live" ? "mixed" : payload.calendar.mode === "demo" ? "mixed" : "fallback" }, h("div", { className: "space-y-5" }, [
