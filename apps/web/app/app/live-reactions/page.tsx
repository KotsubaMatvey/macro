...
export default async function LiveReactionsPage(props: LiveReactionsPageProps) {
 ...
 return h(PageShell, { title: "Reactions", subtitle: "Historical event-family reaction study using the live calendar and provider-backed market history where available.", active: "live-reactions", mode: surfaceMode }, h("div", { className: "space-y-5" }, [
  ...
  h("div", { key: "grid", className: "ws-two-panel" }, [
   ...
   h("div", { key: "right", className: "space-y-5" }, [
    h(Panel, { key: "integrity", title: "Integrity notes", subtitle: "Live and fallback states are surfaced directly in the research view." }, h(DataTable, { headers: ["Field", "Value"], rows: [["Surface mode", surfaceMode], ["Calendar mode", calendarMode], ["Calendar freshness", payload.calendar.freshness], ["Market freshness", payload.summary.freshness.freshness], ["Study note", payload.summary.note]], dense: true })),
