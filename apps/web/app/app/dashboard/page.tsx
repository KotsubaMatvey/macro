...
function statTile(label: string, value: string, note: string) {
 return h("div", { className: "rounded-[14px] border border-white/8 bg-white/[0.03] px-3 py-3" }, [
 h("div", { key: "label", className: "text-[9px] font-semibold uppercase tracking-[0.22em] text-slate-500" }, label),
 h("div", { key: "value", className: "mt-2 ws-mono text-[26px] leading-none text-white" }, value),
...
export default async function DashboardPage(props: any = {}) {
 ...
 const edgeNote = active ? "Model-derived five day edge is " + showPercent(active.expectedMove5dPct) + ", while the source-derived tape is " + showPercent(active.change30dPct) + " over 30d. Risk and liquidity context remain tied to " + payload.keyCatalyst.title + "." : "No live market assets are currently available."
 return h(PageShell, { title: "Dashboard", subtitle: "Desktop macro desk surface with live tape, catalyst linkage, regime context, and honest model labeling.", active: "dashboard" }, h("div", { className: "space-y-4" }, [
 h("div", { key: "context", className: "grid gap-3 xl:grid-cols-[minmax(0,1fr)_auto]" }, [
 h("div", { key: "date", className: "rounded-[16px] border border-white/8 bg-white/[0.03] px-4 py-3" }, [
 h("div", { key: "eyebrow", className: "text-[10px] font-semibold uppercase tracking-[0.22em] text-slate-500" }, "Desk context"),
 h("div", { key: "date-value", className: "mt-2 text-sm font-medium text-white" }, formatDeskDate(payload.generatedAt)),
 h("div", { key: "time-value", className: "mt-1 text-[12px] text-slate-400" }, formatDeskTime(payload.generatedAt) + " refreshed / " + payload.utility.activeSession + " active"),
 ]),
 h("div", { key: "sessions", className: "flex flex-wrap gap-2 rounded-[16px] border border-white/8 bg-white/[0.03] px-3 py-3" }, payload.utility.sessions.map(function (session) {
 return h("div", { key: session.code, className: session.active ? "rounded-full border border-emerald-400/30 bg-emerald-400/10 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.2em] text-emerald-200" : "rounded-full border border-white/8 bg-white/[0.02] px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-500" }, session.code)
 }).concat([h("div", { key: "providers", className: "rounded-full border border-white/8 bg-white/[0.02] px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-400" }, String(providerLive) + " live / " + String(providerLag) + " degraded")]))
 ]),
 h(Panel, { key: "hero", title: "Today edge", subtitle: payload.hero.modelNote, actions: h(Link, { href: payload.keyCatalyst.href, className: "text-sm text-sky-300 transition hover:text-sky-200" }, "Open catalyst") }, active ? h("div", { className: "space-y-4" }, [
 ...
 return h(Link, { key: item.symbol, href: "/app/dashboard?asset=" + item.symbol, className: isActive ? "rounded-full border border-amber-400/30 bg-amber-400/10 px-3 py-1.5 text-[11px] font-medium text-amber-200" : "rounded-full border border-white/10 bg-white/[0.03] px-3 py-1.5 text-[11px] font-medium text-slate-300 transition hover:border-white/20 hover:text-white" }, item.symbol)
 })),
 h("div", { key: "hero-grid", className: "grid gap-4 xl:grid-cols-[minmax(0,1.3fr)_320px]" }, [
 ...
 h("div", { key: "move", className: "mt-2 text-[36px] font-semibold tracking-tight text-white" }, showPercent(active.expectedMove5dPct)),
 h("div", { key: "meta", className: "mt-2 text-sm text-slate-400" }, active.price + " / sample " + String(active.sampleCount) + " / stance " + active.stance),
 ]),
 ...
 h("div", { key: "stats", className: "grid gap-3 md:grid-cols-4" }, [
 ...
 h("p", { key: "note", className: "max-w-4xl text-sm leading-7 text-slate-300" }, edgeNote),
 h("div", { key: "chips", className: "flex flex-wrap gap-2" }, [
 ...
 h("details", { key: "scenarios", className: "rounded-[14px] border border-white/8 bg-[var(--panel-3)] px-4 py-3" }, [
 h("summary", { key: "sum", className: "cursor-pointer text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-300" }, "Scenario distribution"),
 h("div", { key: "rows", className: "mt-4 grid gap-3" }, active.scenarioBuckets.map(function (item) {
 return h("div", { key: item.label, className: "grid gap-2" }, [
 ...
 h("div", { key: "bar", className: "h-2 rounded-full bg-white/[0.05]" }, h("div", { className: "h-full rounded-full bg-amber-300/75", style: { width: String(Math.round(item.probability * 100)) + "%" } })),
 ...
 h("div", { key: "catalyst", className: "ws-note-card" }, [
 h("div", { key: "label", className: "text-[9px] font-semibold uppercase tracking-[0.22em] text-slate-500" }, "Next catalyst preview"),
 h("div", { key: "title", className: "mt-3 text-lg font-semibold text-white" }, payload.keyCatalyst.title),
 h("div", { key: "meta", className: "mt-2 text-[12px] text-slate-400" }, payload.keyCatalyst.country + " / " + payload.keyCatalyst.currency + " / " + payload.keyCatalyst.impact + " impact"),
 h("div", { key: "threshold", className: "mt-3 text-sm text-slate-300" }, payload.keyCatalyst.threshold),
 ...
 h("div", { key: "main-grid", className: "grid gap-4 xl:grid-cols-[minmax(0,1.18fr)_minmax(320px,0.82fr)]" }, [
 h(Panel, { key: "consensus", title: "Market consensus", subtitle: "Bull and bear split is derived from the live dashboard asset basket, and the trend line is a transparent desk proxy built from those same live assets.", actions: h(Link, { href: payload.marketConsensus.href, className: "text-sm text-sky-300 transition hover:text-sky-200" }, "Open market bias") }, h("div", { className: "space-y-4" }, [
 ...
 h("div", { key: "stats", className: "grid gap-3 md:grid-cols-3" }, [
 ...
 h("div", { key: "proxy", className: "rounded-[14px] border border-white/8 bg-[var(--panel-3)] px-4 py-3" }, [
 ...
 return h("div", { key: item.symbol, className: "rounded-[14px] border border-white/8 bg-white/[0.03] px-4 py-3", title: item.note }, [
 ...
 h("div", { key: "bar", className: "mt-3 h-2 rounded-full bg-white/[0.05]" }, h("div", { className: "h-full rounded-full bg-emerald-300/75", style: { width: String(width) + "%" } })),
 ])
 })),
 ])),
 h("div", { key: "stack", className: "grid gap-4" }, [
 h(Panel, { key: "risk", title: "Risk regime", subtitle: payload.riskRegime.interpretation }, regimePanel(payload.riskRegime, "/app/regime-monitor", Math.sign(payload.riskRegime.score) === 1 ? "green" : "sky")),
 h(Panel, { key: "event", title: "Next event", subtitle: "Highest-priority catalyst stays duplicated here so the operator can read urgency and threshold without reopening the hero.", actions: h(Link, { href: payload.keyCatalyst.href, className: "text-sm text-sky-300 transition hover:text-sky-200" }, "Open detail") }, h("div", { className: "space-y-4" }, [
 ...
 h(Panel, { key: "liq", title: "Liquidity regime", subtitle: payload.liquidityRegime.interpretation }, h("div", { className: "space-y-4" }, [
 ...
 h("div", { key: "lower-grid", className: "grid gap-4 xl:grid-cols-[minmax(0,1.05fr)_minmax(320px,0.95fr)]" }, [
 h(Panel, { key: "reactions", title: "Live reactions", subtitle: "Catalyst-linked desk cues are derived from the live tape, current asset stance, and the active dashboard catalyst.", actions: h(Badge, null, String(reactions.length) + " rows") }, h("div", { className: "grid gap-3" }, reactions.map(function (item) {
 return h(Link, { key: item.asset, href: item.href, className: "rounded-[14px] border border-white/8 bg-white/[0.03] px-4 py-3 transition hover:border-white/16 hover:bg-white/[0.05]" }, [
 ...
 h("div", { key: "tag", className: "text-[11px] font-semibold uppercase tracking-[0.2em] " + toneClass(item.verdict) }, item.verdict),
 h("div", { key: "bar-wrap", className: "mt-2 h-2 w-24 rounded-full bg-white/[0.05]" }, h("div", { className: "h-full rounded-full bg-amber-300/75", style: { width: String(item.strength) + "%" } })),
 ]),
 ]),
 ])
 }))),
 h(Panel, { key: "track", title: "AI / model track record", subtitle: payload.trackRecord.note }, h("div", { className: "space-y-4" }, [
 ...
 h("div", { key: "footer-grid", className: "grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_320px]" }, [
 h(Panel, { key: "intel", title: "Linked intelligence", subtitle: "Briefings, official headlines, watchlists, alerts, and catalyst routes stay attached to the desktop dashboard instead of floating as filler cards." }, h("div", { className: "grid gap-3 md:grid-cols-2 xl:grid-cols-3" }, linkedGroups(payload).map(function (group) {
 return h("div", { key: group.label, className: "rounded-[14px] border border-white/8 bg-white/[0.03] px-4 py-3" }, [
 ...
 h(Panel, { key: "providers", title: "Provider state", subtitle: "Desktop freshness stays explicit by provider so the operator can see where the dashboard is live and where it is still fallback." }, h(DataTable, { headers: ["Provider", "Status", "Detail"], rows: providerRows(payload.utility.providers), dense: true })),
 ]),
 ]))
}
