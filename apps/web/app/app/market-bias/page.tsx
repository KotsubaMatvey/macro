...
export default async function MarketBiasPage() { 
 ...
 const metrics = [ 
  ...
  { label: 'Degraded', value: String(payload.providerStatus.degraded), note: payload.summary.freshness.mode + ' / ' + payload.summary.freshness.freshness }, 
 ] 
 ...
 return h(PageShell, { title: 'Market Bias', subtitle: 'Factor influence surface with explicit live and degraded labeling across macro and market inputs.', active: 'market-bias', mode: payload.summary.freshness.mode === 'live' ? 'live' : payload.summary.freshness.mode === 'demo' ? 'demo' : 'fallback' }, h('div', { className: 'space-y-5' }, [ 
