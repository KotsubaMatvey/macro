...
export default async function EventDetailPage(props: { params: Promise<{ eventId: string }> }) {
  ...
  return h(PageShell, { title: event.title, subtitle: event.whyItMatters, active: 'macro-calendar' }, h('div', { className: 'grid gap-5 xl:grid-cols-[minmax(0,1.35fr)_360px]' }, [
    h('div', { key: 'main', className: 'space-y-5' }, [
      ...
      h(Panel, { key: 'briefings', title: 'Linked briefings' }, h('div', { className: 'grid gap-3 text-sm text-slate-300' }, event.linkedBriefings.length ? event.linkedBriefings.map(function (item: any) {
        return h('div', { key: item.id, className: 'rounded-xl border border-white/8 p-4' }, [
          h('div', { key: 'title', className: 'font-medium text-white' }, item.title),
          h('div', { key: 'meta', className: 'mt-1 text-xs uppercase tracking-[0.14em] text-slate-500' }, item.kind + ' / ' + item.analystName),
          h('p', { key: 'summary', className: 'mt-3' }, item.summary),
        ])
      }) : h('div', { className: 'text-slate-500' }, 'No linked briefings yet.'))),
      h(Panel, { key: 'news', title: 'Linked news' }, h('div', { className: 'grid gap-3 text-sm text-slate-300' }, event.linkedNews.length ? event.linkedNews.map(function (item: any) {
        return h('div', { key: item.id, className: 'rounded-xl border border-white/8 p-4' }, [
          h('div', { key: 'title', className: 'font-medium text-white' }, item.title),
          h('div', { key: 'meta', className: 'mt-1 text-xs uppercase tracking-[0.14em] text-slate-500' }, item.source + ' / ' + item.sentiment),
          h('p', { key: 'summary', className: 'mt-3' }, item.summary),
        ])
      }) : h('div', { className: 'text-slate-500' }, 'No linked news yet.'))),
    ]),
    h(Panel, { key: 'meta', title: 'Event context', className: 'h-fit' }, h(KeyValueList, { items: [
